...
- ✅ Alerts 5 day before expiry + on expiry and will keep on alerting until SSL renewal
...
### Create virtual environment and activate (For first time only, ` run_ssl_check.sh` file will do it repeateadly next time)
```bash
python3 -m venv venv
...
## Usage
Run this command to see instant result.
```bash
...
crontab -l
```
Navigate to the EOF and add `0 5 * * * /home/ubuntu/monitoring/ssl-alert/run_ssl_check.sh` It wil keep repeating the script and send alerts. 